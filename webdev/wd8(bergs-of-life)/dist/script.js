...
function draw() {
  ...
  if (gameState === 2) {
    ...
    textSize(16);
    text(
      "A penguin places Icebergs that play Conway's Game of Life.",
      width / 2,
      height / 2 - 100
    );
    text(
      "At the end of each countdown, the Icebergs play Conway's Game of Life ",
      width / 2,
      height / 2 - 75
    );
    text(
      "The penguin needs to be on a table iceberg for each turn.",
      width / 2,
      height / 2 - 50
    );
    text("Move using WASD.", width / 2, height / 2 - 25);
    text(
      "The penguin has eaten nothing but ice for the past 10 days so it poops icebergs ",
      width / 2,
      height / 2
    );

    text("Do not stay on the same iceberg which plays Conway's Game of Life.", width / 2, height / 2 + 75);
    text(
      "The game will end whenever it feels like it",
      width / 2,
      height / 2 + 50
    );
    textSize(25);
    text("Press space to put the penguin out of his misery.", width / 2, height / 2 + 150);
  } else if (gameState === 0) {
    background(255);
    frameRate(1);
    runGOL();
